...
from ..extras.constants import SOE_ID, EOE_ID
        ...
        for elem in elements:
            if isinstance(elem, str):
                if len(elem) != 0:
                    ...
                    while True:
                        soe_index = elem.find("<SOE>", start)
                        eoe_index = elem.find("<EOE>", start)

                        # If no special markers are found, end the loop
                        if soe_index == -1 and eoe_index == -1:
                            break

                        # <SOE>
                        if soe_index != -1 and (eoe_index == -1 or soe_index < eoe_index):
                            # Extract the content before <SOE> and encode it
                            if start < soe_index:
                                part = elem[start:soe_index]
                                token_ids += tokenizer.encode(part, add_special_tokens=False)

                            # Add the special ID for <SOE>
                            token_ids.append(SOE_ID)
                            start = soe_index + len("<SOE>")

                        # <EOE>
                        elif eoe_index != -1:
                            # Extract the content before <EOE> and encode it
                            if start < eoe_index:
                                part = elem[start:eoe_index]
                                token_ids += tokenizer.encode(part, add_special_tokens=False)

                            # Add the special ID for <EOE>
                            token_ids.append(EOE_ID)
                            start = eoe_index + len("<EOE>")
